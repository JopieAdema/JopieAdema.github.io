...
Two extensions are worth flagging even at this pre-policy stage. First, the composition of applicants near the cutoff may itself respond to the rule: an origin just below 20% faces accelerated screening, which could raise its recognition rate the following year if weaker claims are filtered out disproportionately, pushing that origin back above the threshold. Repeated across years, this would produce a zigzag in treatment status -- origins alternating between treated and control as their own recognition rate reacts to the prior year's exposure -- which threatens designs that compare a given origin's treatment status across years, including the difference-in-differences. The RDD is less exposed to this, since it re-identifies the discontinuity within each year's cutoff rather than tracking a given origin's status over time. Second, applicants who prefer to avoid the border procedure altogether have an incentive to evade detection at the border and travel onward to register in a different, typically more western, Member State. That kind of diversion would not show up in recognition-rate composition but would show up in the geographic distribution of first registrations, which is worth examining directly once enough countries report application counts by Member State of registration.
